...
function windowTopics() {
    ...
    tmux split-window 'bash --init-file <(\
        ...
        kafka-topics.sh --create --topic rawtelemetry --replication-factor 2 --partitions 4 --bootstrap-server localhost:9092; \
        kafka-topics.sh --describe --topic rawtelemetry --zookeeper 127.0.0.1:2181; \
    ")'
...
function windowConsumer() {
    ...
    tmux split-window '
        sleep 10 && \
        echo "consuming rawtelemetry" && \
        kafka-console-consumer.sh --topic rawtelemetry --bootstrap-server localhost:9092 \
        '
